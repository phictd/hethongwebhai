...
<div class="phantrang">
	<?php
			...
			if($D != 1){
				$M= $X - $A;
				echo "<a href=index.php?module=hanghoa&act=lhhloai&start=$M&page=$C class=link><<</a>";
			}
			for($i=1;$i<= $C;$i++){
				if($i == $D){
					...
					$M=($i-1)*$A;
					echo " <a href=index.php?module=hanghoa&act=lhhloai&start=$M&page=$C class=link>$i</a> ";
				}
			}
			if($D != $C){
				$M=$X + $A;
				echo "<a href='index.php?module=hanghoa&act=lhhloai&idloai=$item[idLoaiHang]&start=$M&page=$C' class=link>>></a>";
			}
		}
	?>
</div>